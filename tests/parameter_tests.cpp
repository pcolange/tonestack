...
TS_TEST(parameter_log_skew_hits_midpoint) {
    ...
    p.setProportion(0.5f); p.snapshotBlock(512);
    TS_CHECK_NEAR(p.value(), 0.8, 1e-6);

    p.setProportion(1.0f); p.snapshotBlock(512);
    TS_CHECK_NEAR(p.value(), 1.0, 1e-6);

    p.setProportion(0.0f); p.snapshotBlock(512);
...
TS_TEST(parameter_clamps_proportion) {
    ...
    p.setProportion(2.0f);  p.snapshotBlock(512);
    TS_CHECK_NEAR(p.value(), 1.0, 1e-6);
...
TS_TEST(parameter_smoothing_is_block_size_independent) {
    // One second of audio smooths the same amount whether delivered as 100x480 or
    // 480x100 frames — wall-clock smoothing, not block-count smoothing.
    const ParameterDesc d{"x", 0.0f, 1.0f, 0.0f, ParamSkew::Linear, 0.5f, 0.1f};
    Parameter a(d), b(d);
    a.prepare(48000.0);
    b.prepare(48000.0);
    a.setProportion(1.0f);
    b.setProportion(1.0f);
    for (int i = 0; i < 100; ++i) a.snapshotBlock(480);
    for (int i = 0; i < 480; ++i) b.snapshotBlock(100);
    TS_CHECK_NEAR(a.value(), b.value(), 1e-4);
}
